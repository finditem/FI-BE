...
public class S3Service {
    ...
    public List<UploadedImage> uploadWithThumbnail(List<MultipartFile> files) {
        return files.stream().map(this::uploadImageWithThumbnail).toList();
    }
    ...
    private String uploadThumbnailToS3(MultipartFile file) {
        ...
        return s3Client.utilities()
                .getUrl(url -> url.bucket(bucketName).key(s3FileName))
                .toString();
    }
    ...
    public void delete(List<String> imageUrls) {
        ...
        List<String> keys = imageUrls.stream().map(this::getKeyFromImageUrls).toList();
    ...
    private String getKeyFromImageUrls(String imageUrl) {
        try {
            ...
            String decodedKey =
                    URLDecoder.decode(url.getPath(), StandardCharsets.UTF_8); // URI에서 경로 부분을 가져와 URL 디코딩을 통해 실제 키로 변환
