...
public class PushSubscriptionController {
    ...
    @Operation(summary = "푸시 구독 등록",
               description = "브라우저 Service Worker에서 생성한 푸시 구독 정보를 저장합니다. " +
                           "동일한 endpoint가 이미 등록되어 있으면 중복 저장하지 않습니다.")
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    ...
                    content = @Content(
                            ...
                            examples = @ExampleObject(
                                    value = "{\"isSuccess\": false, \"code\": \"USER404-NOT_FOUND\", \"message\": \"사용자를 찾을 수 없습니다.\"}"
                            )
                    )
            )
    })
    ...
    @Operation(summary = "푸시 구독 해제",
               description = "브라우저 푸시 구독을 해제합니다. 로그아웃 시 호출하여 불필요한 푸시 발송을 방지합니다.")
    @ApiResponses({
