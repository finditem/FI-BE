...
public class NotificationController {
    ...
    public SseEmitter subscribe(@AuthenticationPrincipal UserDetails userDetails) {
        ...
        // 재연결 시 미읽은 알림 전송
        sseEmitterService.sendUnreadNotifications(user.getId(), notificationService.getUnreadNotificationDTOs(user));
    ...
    @Operation(
            summary = "내 알림 목록 조회",
            description = "커서 기반 페이지네이션으로 알림을 조회합니다. cursor를 생략하면 최신 항목부터 반환합니다. 읽음/읽지 않음 및 알림 타입별 필터링 가능")
    @ApiResponses({
    ...
    public ApiResponse<CursorPageResponse<NotificationListDTO>> getMyNotifications(
        ...
        User user = userRepository
                .findByEmail(userDetails.getUsername())
                .orElseThrow(() -> new GeneralException(ErrorStatus._USER_NOT_FOUND));
        CursorPageResponse<NotificationListDTO> notifications =
                notificationService.getMyNotificationsCursor(user, unreadOnly, notificationType, cursor, size);
                ...
                content =
                        @Content(
                                ...
                                examples =
                                        @ExampleObject(
                                                value =
                                                        "{\"isSuccess\": false, \"code\": \"NOTIFICATION404-NOT_FOUND\", \"message\": \"존재하지 않는 알림입니다.\"}")))
    })
    public ApiResponse<String> markAsReadBatch(
            @AuthenticationPrincipal UserDetails userDetails, @RequestBody IdsRequest request) {
        User user = userRepository
                .findByEmail(userDetails.getUsername())
                .orElseThrow(() -> new GeneralException(ErrorStatus._USER_NOT_FOUND));
                ...
                content =
                        @Content(
                                ...
                                examples =
                                        @ExampleObject(
                                                value =
                                                        "{\"isSuccess\": false, \"code\": \"NOTIFICATION403-ACCESS_DENIED\", \"message\": \"해당 알림에 접근할 권한이 없습니다.\"}"))),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
                responseCode = "404",
                description = "NOTIFICATION404-NOT_FOUND: 존재하지 않는 알림입니다",
                content =
