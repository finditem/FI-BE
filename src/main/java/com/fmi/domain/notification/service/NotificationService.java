...
public class NotificationService {
     ...
     * 내 알림 목록 조회
     */
    public Page<NotificationListDTO> getMyNotifications(
            User user, Boolean unreadOnly, NotificationType notificationType, Pageable pageable) {
     ...
     * 내 알림 목록 조회 (커서 기반)
     */
    public CursorPageResponse<NotificationListDTO> getMyNotificationsCursor(
            User user, Boolean unreadOnly, NotificationType notificationType, Long cursor, int size) {
        ...
        if (notificationType != null && Boolean.TRUE.equals(unreadOnly)) {
            notifications =
                    notificationRepository.findByUserAndTypeAndIsReadFalseCursor(user, notificationType, cursor, limit);
        } else if (notificationType != null) {
        ...
        List<NotificationListDTO> responseList =
                content.stream().map(notificationConverter::toListDTO).toList();
    ...
    public List<NotificationListDTO> getUnreadNotificationDTOs(User user) {
        ...
        return unread.stream().map(notificationConverter::toListDTO).toList();
    }
    ...
    public void markNotificationsAsReadByReference(User user, Long referenceId, List<NotificationType> types) {
        List<Notification> notifications =
                notificationRepository.findAllByUserAndReferenceIdAndTypeIn(user, referenceId, types);
    ...
    public NotificationSettingsDTO getSettings(User user) {
        NotificationSettings settings =
                notificationSettingsRepository.findByUser(user).orElseGet(() -> createDefaultSettings(user));
    ...
    public NotificationSettingsDTO updateSettings(User user, NotificationSettingsUpdateDTO request) {
        ...
        NotificationSettings settings =
                notificationSettingsRepository.findByUser(freshUser).orElseGet(() -> createDefaultSettings(freshUser));
    ...
    @Deprecated
    @Transactional
    public NotificationSettingsDTO updateBasicSettings(
            User user, Boolean commentEnabled, Boolean chatEnabled, Boolean categoryEnabled) {
        // User 엔티티를 최신 상태로 조회
        User freshUser = userRepository
                .findById(user.getId())
                .orElseThrow(() -> new GeneralException(ErrorStatus._USER_NOT_FOUND));
    ...
    public void createNotification(
        ...
        // 알림 설정 확인
        NotificationSettings settings =
                notificationSettingsRepository.findByUser(user).orElse(null);
            ...
            if (TransactionSynchronizationManager.isSynchronizationActive()) {
                TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
                    ...
                    public void afterCommit() {
                        try {
                            // SSE로 실시간 푸시
                            sseEmitterService.sendToUser(user.getId(), notificationConverter.toListDTO(notification));
                            log.info(
                                    "SSE 알림 전송 시도: userId={}, notificationId={}",
                                    user.getId(),
        ...
        do {
            ...
            for (User user : users) {
                ...
                if (settings == null || Boolean.TRUE.equals(settings.getNoticeEnabled())) {
                    createNotification(
                            user, NotificationType.NOTICE, "새로운 공지사항이 등록되었습니다.", title, ReferenceType.NOTICE, noticeId);
                }
            }
        } while (users.hasNext());
    }
    ...
    public void notifyCategoriesForPost(Post post) {
        ...
        for (var userCategory : subscribedUsers) {
            ...
            if (post.getUser() != null
                    && subscriber.getId().equals(post.getUser().getId())) {
                continue;
            }

            // 알림 설정 확인 및 발송
            NotificationSettings settings =
                    notificationSettingsRepository.findByUser(subscriber).orElse(null);
            ...
            createNotification(subscriber, NotificationType.CATEGORY, title, message, ReferenceType.POST, post.getId());
        }

        log.info(
                "카테고리 알림 발송 완료: postId={}, category={}, subscribers={}",
                post.getId(),
                post.getCategory(),
                ...
                public void afterCommit() {
                    try {
                        ...
                        sseEmitterService.sendToUser(
                                notification.getUser().getId(), notificationConverter.toListDTO(notification));
                        log.info(
                                "SSE 알림 갱신 전송: userId={}, notificationId={}",
                                notification.getUser().getId(),
                                notification.getNotificationId());
                    } catch (Exception e) {
                        log.warn(
                                "SSE 알림 갱신 전송 실패: userId={}",
                                notification.getUser().getId(),
                                e);
                    }
                    // Web Push 발송
                    try {
                        String url =
                                buildNotificationUrl(notification.getReferenceType(), notification.getReferenceId());
