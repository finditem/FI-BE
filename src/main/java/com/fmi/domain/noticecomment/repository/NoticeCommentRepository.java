...
public interface NoticeCommentRepository extends JpaRepository<NoticeComment, Long> {
    ...
    Slice<NoticeComment> findByNoticeIdAndIdLessThanOrderByIdDesc(
            @Param("noticeId") Long noticeId, @Param("cursor") Long cursor, Pageable pageable);

    @Query(
            value = "select c from NoticeComment c join fetch c.user "
    ...
    @Query(
            "SELECT c.notice.noticeId, COUNT(c) FROM NoticeComment c WHERE c.notice.noticeId IN :noticeIds GROUP BY c.notice.noticeId")
