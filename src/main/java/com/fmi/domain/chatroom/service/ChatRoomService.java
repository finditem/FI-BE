...
public class ChatRoomService {
    ...
    public Pair<ChatRoomResultDTO, Boolean> createChatRoom(Long postId, Long contactUserId) {
        ...
        // 기존 채팅방이 있는지 조회
        Optional<ChatRoom> optionalChatRoom = chatRoomRepository.findChatRoomByPostAndUsers(
                postId, post.getUser().getId(), contactUserId);
        ...
        if (optionalChatRoom.isPresent()) {
            ...
            ChatRoomResultDTO dto = ChatRoomConverter.toChatRoomResultDTO(
                    existingRoom, opponentUser, post, unreadCount, thumbnailImageUrl, isBlocked);
        ...
        // 채팅방이 존재하지 않는 경우 (새로 생성)
            ...
            User user = userRepository
                    .findActiveById(contactUserId)
                    .orElseThrow(() -> new GeneralException(ErrorStatus._USER_NOT_FOUND));
            User postAuthor = post.getUser();
            ChatRoom newRoom =
                    ChatRoom.builder().post(post).createdAt(LocalDateTime.now()).build();
            newRoom.initFromPost(post);

            // 두 명의 참여자(게시글 작성자, 연락한 사람)에 대한 ChatRoomParticipant 엔티티를 생성
            ChatRoomParticipant authorParticipant =
                    ChatRoomParticipant.builder().user(postAuthor).build();

            ChatRoomParticipant contactUserParticipant =
                    ChatRoomParticipant.builder().user(user).build();
            ...
            ChatRoomResultDTO dto = ChatRoomConverter.toChatRoomResultDTO(
                    newRoom, opponentUser, post, 0L, thumbnailImageUrl, isBlocked);

            return Pair.of(dto, true);
        }
    }

    public MyChatListDTO getMyPostChatRooms(
            User user, Long cursorId, int size, PostType type, String address, SortType sort) {

        PageRequest pageable = PageRequest.of(0, size);

        Slice<ChatRoomParticipant> participantSlice =
                chatRoomParticipantRepository.findMyChatRooms(user.getId(), cursorId, pageable, type, address, sort);
    ...
    private MyChatListDTO buildChatListResponse(Slice<ChatRoomParticipant> participantSlice, User user) {
        ...
        List<ChatRoomSummaryDTO> summaryDTOs =
                ChatRoomConverter.toChatRoomSummaryListDTO(participants, user, thumbnailMap);
    ...
    public void leaveChatRoom(Long roomId, Long userId) {
        ChatRoomParticipant chatRoomParticipant = chatRoomParticipantRepository
                .findByUser_IdAndChatRoom_Id(userId, roomId)
        ...
        Long lastMsgId = chatMessageRepository
    ...
    public ChatRoomResultDTO getChatRoomDetail(Long roomId, User user) {
        ...
        if (!chatRoom.isSourcePostDeleted() && chatRoom.getPost() != null) {
            ...
            String thumbnailImageUrl = postImageService.findThumbnailImageUrl(post);
            return ChatRoomConverter.toChatRoomResultDTO(
                    chatRoom, opponent, post, unreadCount, thumbnailImageUrl, isBlocked);
        }
